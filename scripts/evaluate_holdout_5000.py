...
PRED_CSV = OUTPUT_DIR / "holdout_5000_predictions.csv"
THRESHOLD_CSV = OUTPUT_DIR / "holdout_5000_threshold_sweep_any.csv"
TRIAGE_CSV = OUTPUT_DIR / "holdout_5000_triage_simulation.csv"
METRICS_JSON = OUTPUT_DIR / "holdout_5000_eval_metrics.json"
...
def triage_simulation(pred_df):
    """
    Simulate radiology prioritization:
    sort all CT cases by prob_any descending, then calculate how many true hemorrhage
    cases are captured in the top X% workload.
    """
    ...
    for top_percent in [1, 2, 5, 10, 15, 20, 25, 30, 40, 50]:
        ...
        recall_at_k = captured_positive / max(total_positive, 1)
        precision_at_k = captured_positive / max(k, 1)
...
@torch.no_grad()
def main():
    print("===== EVALUATE BEST MODEL ON NATURAL HOLDOUT 5000 =====")
    ...
    print("\nSTATUS: NATURAL HOLDOUT 5000 EVALUATION PASSED")
